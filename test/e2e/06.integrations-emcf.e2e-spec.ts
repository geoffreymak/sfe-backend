...
describe('Integrations - eMCF (E2E)', () => {
  ...
  beforeAll(async () => {
    ...
    const res = await http(app)
      ...
      .send({
        ...
        organizationName: 'Org eMCF ' + rand(),
      })
      .expect(201);
  ...
  it('PUT /settings sets integration mode to emcf', async () => {
    const payload = {
      integration: {
        mode: 'emcf',
        emcf: {
          baseUrlInfo: 'http://emcf.local',
          isf: 'ACME-ISF',
          nif: 'A1234567',
          token: 'secret-token',
        },
    ...
    expect(res.body?.integration?.mode).toBe('emcf');
  });
  ...
  it('POST /invoices/:id/normalize (idempotent) and updates security', async () => {
    ...
    // Second call returns cached body (same status here); ensure body equality
    ...
    // Shape assertions
    expect(first.body?.mode).toBe('emcf');
    expect(first.body?.invoiceId).toBe(invId);
    expect(first.body?.source).toBe('emcf');
    ...
    // Invoice reflects security
    const inv = await authed().get(`/invoices/${invId}`).expect(200);
    expect(inv.body?.security?.source).toBe('emcf');
    expect(typeof inv.body?.security?.codeDefDgi).toBe('string');
  });

  it('GET /integrations/status returns emcf status ok', async () => {
    const res = await authed().get('/integrations/status').expect(200);
    expect(res.body?.mode).toBe('emcf');
    expect(res.body?.emcf?.ok).toBe(true);
  });
});
